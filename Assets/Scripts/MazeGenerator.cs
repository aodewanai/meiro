...
public class MazeGenerator 
{
   ...
   public MazeGeneratorCell[,] GeneraneMaze()
   {
    ...
    private void RemoveWalls(MazeGeneratorCell[,] maze)
    {
        ...
        do
        {
            ...
            if(unvisitedNeighbours.Count > 0)
            {
                ...
                stack.Push(chosen);
                current = chosen;
                chosen.DistanseFromStart = stack.Count;
            }
            else
            {
                current = stack.Pop();
    ...
    private void PlaceMazeExit(MazeGeneratorCell[,] maze)
    {
        ...
        for (int x = 0; x < maze.GetLength(0); x++)
        {
            if (maze[x, Height - 2].DistanseFromStart > furthest.DistanseFromStart) furthest = maze[x, Height - 2];
            if (maze[x, 0].DistanseFromStart > furthest.DistanseFromStart) furthest = maze[x, 0];
        }
        for(int y = 0; y < maze.GetLength(1); y++)
        {
            if (maze[Width - 2, y].DistanseFromStart > furthest.DistanseFromStart) furthest = maze[Width - 2, y];
            if (maze[0, y].DistanseFromStart > furthest.DistanseFromStart) furthest = maze[0, y];
        }
